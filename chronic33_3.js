...
const isnurance_wrap_div = document.querySelector("#isnurance_wrap");
...
const validateSubmit = () => {
  if (validateStateInput()) {
    if (validState && validName && validPhone && validProvider) {
      next_btn.disabled = false;
    } else {
      next_btn.disabled = true;
    }
  } else {
    if (validState && validName && validPhone) {
      next_btn.disabled = false;
    } else {
      next_btn.disabled = true;
    }
...
const selectionOfState = (event) => {
  ...
  if (validateStateInput()) {
    // step = "insurance";
    isnurance_wrap_div.style.display = "block";
    setInsuranceOptions();
  } else {
    // step = "not_insured";
    input_insurance_hidden.value = "";
    isnurance_wrap_div.style.display = "none";
  }
};
...
input_phone.addEventListener("input", () => reset());
...
const lp_value = urlParams.get("lp") || "68";
...
const formSubmition = () => {
  ...
  const moveNext = () => {
    ...
    gtag("event", "signup_flow_33_submit");
    // console.log("signup_flow_33_submit");
    vwoEvent();
...
const handleNextButtonsLogic = () => {
  if (
    ...
    if (
      ...
      gtag("event", "signup_flow_33_insurance_not_accepted");
      // console.log("signup_flow_33_insurance_not_accepted");
    } else {
      gtag("event", "signup_flow_33_state_not_accepted");
      // console.log("signup_flow_33_state_not_accepted");
    }
  }
};
